...
export class WorkspacesService {
  ...
  async listWorkspaces(
  ...
  ): Promise<PaginatedResult<WorkspaceView>> {
    const db = this.getDatabase();
    const conditions = [eq(workspaces.organizationId, input.organizationId)];

    if (input.status) {
      conditions.push(eq(workspaces.status, input.status));
    }
    ...
    return paginateResult(
      items.map((item) => this.serializeWorkspace(item)),
      countRows[0]?.count ?? 0,
      input.pagination,
    );
  }
